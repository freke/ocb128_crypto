...
pack(cryptState_t *cs, ErlNifEnv* env)
{
    ...
    memcpy(enif_make_new_binary(env, 0xff, &history), cs->decrypt_history, 0xff);
...
unpack(cryptState_t *cs, const ERL_NIF_TERM tuple[], ErlNifEnv* env)
{
    ...
    if(!enif_inspect_binary(env, tuple[3], &p_history) || p_history.size != 0xff)
    {
        return FALSE;
    }
...
encrypt_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if(!enif_get_tuple(env, argv[0], &ar, &tuple) || ar != 5 || !enif_inspect_binary(env, argv[1], &p_source))
    {
    ...
    CryptState_encrypt ( &cs, p_source.data, enif_make_new_binary(env, p_source.size + 4, &encrypted), p_source.size );
...
decrypt_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if(CryptState_decrypt ( &cs, p_source.data, enif_make_new_binary(env, p_source.size - 4, &decrypted), p_source.size ) != TRUE)
    {
        return enif_make_tuple2(env, enif_make_atom(env, "error"), pack(&cs, env));
    }
...
ERL_NIF_INIT(ocb128_crypto_port, nif_funcs, load, NULL, upgrade, unload)
